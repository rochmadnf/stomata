<div class="mb-5">
    ...
    <input type="{{ $type ?? 'text' }}" name="{{ $name }}" id="{{ $name }}" @class([
        'block w-full rounded-md border border-green-200 bg-slate-100 p-2.5 text-sm outline-none focus:border-green-400 focus:ring-green-400',
        'bg-red-50 border-red-500 focus:ring-red-500 focus:border-red-500' => $errors->has(
            $name),
    ])
        placeholder="{{ $placeholder ?? '' }}" value="{{ old($name) ?? '' }}" @readonly($readonly ?? false)>
