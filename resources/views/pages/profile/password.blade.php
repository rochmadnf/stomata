<x-main-layout subtitle="ubah katasandi">
    <div class="rounded-lg p-4">
        <h1 class="mb-4 text-2xl font-bold text-slate-800 lg:mb-8">Ubah Katasandi</h1>

        <div class="rounded-md bg-white px-4 py-2">
            <form id="fuPaUpdate" action="{{ route('profile.update.password', ['id' => $id]) }}" method="POST"
                class="space-y-6">
                @csrf
                <x-forms.input label="Katasandi Lama" name="old_password" type="password" :required="true" />

                <x-forms.input label="Katasandi Baru" name="password" type="password" :required="true" />
                <x-forms.input label="Konfirmasi Katasandi Baru" name="password_confirmation" type="password"
                    :required="true" />
    ...
    <x-slot name='scripts'>
        <script>
            document.querySelector('#fuPaUpdate').addEventListener('submit', () => {
